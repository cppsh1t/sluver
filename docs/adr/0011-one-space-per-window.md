...
- **Corollary — window label tier must match route tier (no cross-tier navigation)**: a Tauri window's label is fixed at creation and is the single source of truth for the tray menu and the single-instance check. A window may therefore only ever render routes of its OWN tier — launcher-scoped routes (`/`, `/settings`) in the `"main"` window, and `/space/{id}/…` routes in a `space-{id}` window. Client-side `navigate()` MUST NOT cross these tiers inside an existing window: it changes the URL (and thus the rendered layout) without changing the label, orphaning the label from the content. This invariant was violated twice and each time fixed by eliminating the cross-tier navigation rather than permitting it:
  - **Space → Space** (`CreateSpaceDialog`): after creating a Space it `navigate`d the current window to `/space/{newId}`, but the label stayed `space-{oldId}` (or `"main"`), so the tray still listed the old Space and the new Space never appeared. Fixed by routing both the Space picker and the create dialog through the `useOpenSpaceInWindow` hook (composes `openSpace` + `openSpaceWindow`) — the ONLY correct way to present a Space.
  - **Space → launcher** (`AppSidebar` footer "Settings"): the `<Link to="/settings">` navigated a Space window to the launcher-scoped `/settings` route, swapping in the landing `AppSidebar` over Space content while the label stayed `space-{id}`. Fixed by converting Settings into a `SettingsDialog` with no route at all — global preferences now open as a modal from any window and never navigate.
