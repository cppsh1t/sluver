...
import { getConversation, updateConversationTitle } from "@/api";
...
export function ConversationRuntimeProvider({
   ...
   * conversation. Flow: kind-agnostic single-fetch DB pre-check via
   * `getConversation` (bails BEFORE spending an LLM call when a title is
   * already set — e.g. a sidebar rename the slot cache never saw) →
   * `generateConversationTitle` → post-generation race re-check →
