 ...
 * the parent route. Per ADR-0050 D1 there is no role picker anymore: every
 * new user-facing conversation is bound to the Orchestrator (the only
 * conversational role). Rows support inline rename (hover pencil or
 * double-click the title; Enter/blur commits, Escape cancels — same
 * pattern as the Notes tree).
 */
...
export function ConversationList({
  ...
  /**
   * Create a new conversation. Per ADR-0050 D1 there is exactly one
   * conversational role — every user-facing conversation is
   * orchestrator-bound (subagent runs are created by the dispatch tool,
   * never here).
   */
