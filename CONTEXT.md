...
**AgentConfig**:
A named AI configuration slot within a Space — the persistent definition of an AgentLoop's model and behavior. Carries the bound model (chosen from the Space's configured providers), the `autoExecuteDangerousTools` flag (whether configurable-level tools auto-execute without user approval), and the `shellToolEnabled` flag (default-off opt-in that registers the role's shell tool, gated per-config — to auto-execute without per-call approval; see ADR-0042); the rest of the behavior bundle (system prompt, tools, parameters) is hardcoded in code via `AgentLoopOptions`. Eleven AgentConfigs are seeded per Space — the **Orchestrator**, the eight **Subagents** (explorer, curator, scribe, historian, editor, plotter, writer, critic), and the non-conversational `namer`/`vision` — into `space.db` on Space creation; users pick a model for each but cannot create or delete AgentConfigs. An unconfigured role is never hidden: dispatching it fails explicitly with a reportable `unconfigured` status rather than silently omitting it.
...
**Subagent**:
A non-user-facing specialist role that only the Orchestrator can dispatch. Eight are seeded per Space — explorer, curator, scribe, historian, editor, plotter, writer, critic — each with its own AgentConfig (model binding, prompt override, consent flags, skill pool). Exactly one delegation level: Subagents cannot dispatch further.
_Avoid_: Child Agent, Worker, Minion, Delegate, Helper

**Dispatch**:
One act of the Orchestrator spawning a Subagent Run — a single tool call carrying the target Subagent and a free-form task brief composed by the Orchestrator (including any context carried over from earlier runs). Sibling Dispatches issued in the same step run concurrently; the Orchestrator's turn blocks until all of them resolve.
_Avoid_: Spawn, Fork, Delegate, Assignment

**Subagent Run**:
One execution instance of a Dispatch — a fresh, one-shot, machine-authored Conversation (kind "subagent") with its own transcript, tool loop, step budget, and terminal status (`completed` / `aborted` / `stopped` / `error` / `unconfigured`). Runs share no memory; all cross-run context flows through the Orchestrator's task briefs. The user may inspect (drill into the transcript) and stop a Run, but never speaks in it.
_Avoid_: Child Session, Task Run, Worker, Sub-chat

**ConsentLevel**:
The safety classification declared on each agent tool definition, governing whether the user must explicitly approve a tool call before it executes. Three levels: **auto** — read-only operations (list, get, count) execute without asking; **configurable** — create operations execute without asking only if the agent's `autoExecuteDangerousTools` flag is enabled, otherwise require approval; **always** — edit, delete, and reorder operations always require explicit approval regardless of configuration. Declared per-tool in the tool definition, with per-role overrides recorded in the role registry (notably the writer Subagent's `update_scene` is `configurable`, unlocking the unattended batch-writing pipeline; delete/reorder stay `always` for every role); not derivable from operation type alone.
_Avoid_: PermissionLevel, ApprovalLevel, TrustLevel, DangerLevel, RiskLevel

**Conversation**:
A persisted, World-scoped chat history bound to one AgentConfig for its entire lifetime (the user-facing counterpart of the library's `SessionRecord`, ADR-0020 — the pure library is domain-agnostic and calls it a "Session"; the app layer wraps and types it as a Conversation). User-facing Conversations hold turns between the user and the Orchestrator; a Subagent Run is also a Conversation, authored by a single Dispatch brief instead of user turns. Contains an ordered list of **Messages**. World-scoped per ADR-0022 — a Conversation belongs to exactly one World and is physically invisible to other Worlds (it lives in that World's `world.db`). Conversations come in three kinds, distinguished by `meta.kind`: **World-level** (`kind: "world"`, listed in the World's Chat page), **chapter-scoped** (`kind: "chapter"` with a `chapterId`, shown inside that chapter's editor), and **Subagent Runs** (`kind: "subagent"` — machine-authored, carrying parent linkage, hidden from the chat list). The user-facing kinds are one-to-many — a World holds many World-level Conversations, and a Chapter holds many chapter-scoped Conversations. The UI feature label is "Chat" — "Chat" is a UI affordance over Conversations, not a domain term.
