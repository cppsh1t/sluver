...
- **Web search & fetch** (`commands/search.rs`): Bing results-page HTML parsing (`scraper` + `url`, including unwrapping `/ck/a?` tracking redirects), main-article extraction (`readabilityrs`), and on Windows a hidden-WebView2 fetch path (`webview2-com` + `windows` crates, tokio-coordinated — Tauri's `eval()` is fire-and-forget and can't return JS results). The builtin SERP engines run on Windows AND Linux: a hidden WebKitGTK window + `webkit2gtk::evaluate_javascript` bridge (`webkit2gtk`/`javascriptcore-rs` Linux target deps) provides the ExecuteScript equivalent — see `docs/web-search-builtin-engines-platform-support.md`.
