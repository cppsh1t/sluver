...
- [ADR-0034](./docs/adr/0034-timeline-ui-uniform-character-swimlane-grid.md) — Timeline UI as uniform character-swimlane grid (non-proportional, order-based; supersedes ADR-0033 UI surface)
  ...
  - [ADR-0052](./docs/adr/0052-image-download-webview-fallback.md) — Transparent image-download WebView2 fallback: `download_image_bytes_with_fallback` (`commands/image.rs`) retries HTTP 401/403/429 only via `commands/search.rs::download_image_bytes_via_webview` (hidden `webview-image-{uuid}` window, same-origin `fetch` polled into `window.__sluverImg`); Windows + Linux via the platform webview (amended 2026-09-19, riding the WebKitGTK bridge), tool surface untouched, original HTTP error preserved when the fallback also fails
  - [ADR-0053](./docs/adr/0053-chapter-scoped-conversations.md) — Chapter-scoped conversations in the chapter editor (realizes ADR-0022's reservation; dedicated per-chapter listing command, construction-time `<chapter_context>` injection, two-level cascade sweep on chapter/novel deletion)
  - [ADR-0054](./docs/adr/0054-linux-frontend-caption-buttons.md) — Linux renders frontend caption buttons; decorum `create_overlay_titlebar()` is Windows/macOS-only (decorum 1.1.1's Linux injection is broken: dead `icon:` button, duplicated sets, ReferenceError in minimize)
  - [ADR-0055](./docs/adr/0055-official-skills.md) — Official skills: compile-time-embedded packages seeded into every Space at connection open (`skills.kind` discriminator, space v14), undeletable (`SKILL_OFFICIAL_PROTECTED`), reserved names (`SKILL_NAME_RESERVED`), default-enabled per role via junction-once seeding (curator ← element-creation-guide, orchestrator ← subagent-dispatch-guide); lazy disk materialization self-healed at first `activate_skill`; DB-only content refresh
...
- **Web search & fetch** (`commands/search.rs`): Bing results-page HTML parsing (`scraper` + `url`, including unwrapping `/ck/a?` tracking redirects), main-article extraction (`readabilityrs`), and a hidden-platform-webview machinery (`eval_js_string`: WebView2 `ExecuteScript` via `webview2-com` + `windows` on Windows; WebKitGTK `evaluate_javascript` via `webkit2gtk`/`javascriptcore-rs` on Linux — tokio-coordinated both sides, since Tauri's `eval()` is fire-and-forget and can't return JS results) that powers `fetch_url_via_webview`, the builtin SERP engines, and the ADR-0052 image-download fallback on Windows AND Linux; macOS stubs out — see `docs/web-search-builtin-engines-platform-support.md`.
...
- **Window chrome & native dialogs**: `tauri-plugin-decorum` (frameless windows + snap overlay, used by `window_manager.rs` and the custom TitleBar — ADR-0009; on Linux the overlay injection is skipped and `WindowTitleBar` renders its own caption buttons — ADR-0054) and `tauri-plugin-dialog` (native directory picker for log export).
