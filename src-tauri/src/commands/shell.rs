...
mod tests {
    ...
    fn shell_error_payload_codes() {
        let p = ShellError::ParseError("unexpected token ')'".into())
            .to_payload();
        assert_eq!(p.code, "SHELL_PARSE_ERROR");
        assert_eq!(p.message, "unexpected token ')'");
        assert!(p.args.is_empty());

        let p = ShellError::InvalidInput("cwd must be absolute".into())
            .to_payload();
    ...
    fn shell_error_serializes_as_payload() {
        let json = serde_json::to_value(ShellError::ParseError("bad".into()))
            .expect("serialize");
    ...
    fn truncate_under_cap_is_noop() {
        let (out, truncated, len) =
            truncate_output("hello world".to_string());
